...
| `PBR-OBSERVER-021` | Tier 1 current exact source independently approved; hosted admission pending | Exact source `af9f77d` preserves trace startup and permanently rejects an unreconciled process tree. Native ptrace behavior and complete observation remain open. |
| `PBR-OBSERVER-022` | Tier 1 current exact source independently approved; hosted admission pending | Exact source `af9f77d` extends the setup adapter with active and drain states under one private ownership chain. |
| `PBR-OBSERVER-023` | Tier 1 current exact source independently approved; hosted admission pending | Exact source `af9f77d` preserves the active-trace event source and makes registration failure publication-fatal. Kernel effects and decoding remain open. |
| `PBR-OBSERVER-024` | Tier 1 corrected live event-and-drain source independently approved; hosted admission pending | The separate adapter consumes complete trace events into the matching pure protocol, transfers failures and bounds into a drain-only state, rejects an unreconciled tree, and gates pure tree-empty acknowledgement on successful effectful drain reconciliation. |
