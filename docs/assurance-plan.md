...
| `PBR-OBSERVER-025` | Tier 1 syscall-decoder source implementation; lint correction exact review and hosted admission pending | Approved source `5d82dcd` reached hosted run `34979197795`; formal, native, and fresh-evidence lanes passed, but Rust lint rejected its large private enum. Correction `5d479bd` uses `Option<ActiveTraceEvent>` without per-event allocation and refreshes exact source guards. Kernel ABI truth, memory stability, native attacks, and artifact mapping remain open. |
