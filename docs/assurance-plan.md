...
| `PBR-OBSERVER-020` | Tier 1 admitted baseline; prior coupling approval is historical after decoder changes | Exact source `af9f77d` passed independent review. This batch changes the observer source to expose validated capture limits, so the current exact identity is pending combined review and hosted admission. |
| `PBR-OBSERVER-021` | Tier 1 admitted trace-startup baseline; prior coupling approval is historical after decoder changes | Exact source `af9f77d` passed independent review. This batch changes the shared trace source, so the current exact identity is pending combined review and hosted admission. |
| `PBR-OBSERVER-022` | Tier 1 prior setup and coupling approvals are historical after decoder changes | This batch changes the adapter release transition to derive capture limits from the same validated protocol. The current exact identity is pending combined review and hosted admission. |
| `PBR-OBSERVER-023` | Tier 1 prior active-trace and coupling approvals are historical after decoder changes | This batch changes `ActiveTrace` to decode and capture operands at entry. The current exact identity is pending combined review and hosted admission. |
| `PBR-OBSERVER-024` | Tier 1 prior coupling approval is historical after decoder changes | The approved `af9f77d` drain correction remains history. The decoder changes shared trace, adapter, and observer sources, so current exact identity is pending combined review and hosted admission. |
