...
usage='usage: run_resolution_indirection.sh <landlock-port|cgroup-endpoint|explicit-broker|preconnected-channel> <new-absolute-result-directory>'
...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo 'resolution indirection experiment requires native Linux' >&2
  exit 3
fi
...
if [[ "$1" != "--inside" ]]; then
  ...
  case "$mechanism" in
    ...
    *) echo 'resolution indirection mechanism is invalid' >&2; exit 2 ;;
  esac
  ...
  if [[ $EUID -ne 0 ]]; then
    echo 'resolution namespace setup requires root; invoke this script through sudo' >&2
    exit 3
  fi
  if [[ -n "$(git -C "$repository_root" status --porcelain)" ]]; then
    echo 'resolution experiment requires a clean exact Git commit' >&2
    exit 2
  fi
...
if [[ "$repository_root" != "$expected_root" || $EUID -ne 0 ]]; then
  echo 'resolution namespace handoff identity mismatch' >&2
  exit 4
fi
case "$mechanism" in
  landlock-port|cgroup-endpoint|explicit-broker|preconnected-channel) ;;
  *) echo 'resolution namespace mechanism changed' >&2; exit 4 ;;
esac
cd "$repository_root"

for command in cc chown cmp cp git ip openssl python3 stat; do
  if ! command -v "$command" >/dev/null 2>&1; then
    echo "resolution experiment prerequisite is unavailable: $command" >&2
    exit 3
  fi
done
if [[ "$mechanism" == "cgroup-endpoint" ]]; then
  if [[ "$(stat -f -c %T /sys/fs/cgroup)" != "cgroup2fs" ]]; then
    echo 'resolution endpoint experiment requires cgroup v2' >&2
    exit 3
  fi
  if [[ ! -f /sys/kernel/btf/vmlinux || -L /sys/kernel/btf/vmlinux ]]; then
    echo 'resolution endpoint experiment requires regular kernel BTF' >&2
    exit 3
  fi
fi
...
staged_client="$staged_network/resolution_network_client.py"
...
openssl req \
  ...
  -out "$artifact_root/allowed-certificate.pem" \
  >"$private_root/certificate.stdout" \
  2>"$private_root/certificate.stderr"
chmod 0600 "$private_root/allowed-key.pem"
chmod 0644 "$artifact_root/allowed-certificate.pem"

python3 -c \
  'import sys; from pathlib import Path; from experiments.network_authority.record_common import canonical_json,write_new; write_new(Path(sys.argv[1]), canonical_json({"address":"127.0.0.1","policy":"registered","schema":"proofbound-runtime-resolver-config/1"})); write_new(Path(sys.argv[2]), canonical_json({"address":"127.0.0.1","policy":"substitute-undeclared","schema":"proofbound-runtime-resolver-config/1"}))' \
  "$artifact_root/registered-resolver.json" \
  "$artifact_root/substitute-resolver.json"

mapfile -t resolution_cases < <(
  python3 -c \
    'from pathlib import Path; from experiments.network_authority.resolution_indirection_case import load_resolution_matrix; matrix=load_resolution_matrix(Path("experiments/network_authority/decision-matrix.toml").resolve()); print("\n".join(case.identifier for case in matrix.cases))'
)
if [[ ${#resolution_cases[@]} -ne 18 ]]; then
  echo 'resolution case inventory is not exact' >&2
  exit 4
fi
...
for case_id in "${resolution_cases[@]}"; do
  ...
  common_arguments=(
    ...
    --registered-resolver "$artifact_root/registered-resolver.json"
    --substitute-resolver "$artifact_root/substitute-resolver.json"
  )
  set +e
  case "$mechanism" in
    landlock-port)
      python3 -m experiments.network_authority.run_resolution_direct_case \
        "${common_arguments[@]}" \
    ...
    cgroup-endpoint)
      cgroup_arguments=()
      if [[ "$case_id" != "resolver-configuration-substitution" ]]; then
        cgroup_directory="$cgroup_parent/proofbound-resolution-$$-${#cgroup_directories[@]}"
      ...
      ;;
    explicit-broker)
      python3 -m experiments.network_authority.run_resolution_broker_case \
        "${common_arguments[@]}" \
      ...
      ;;
    preconnected-channel)
      python3 -m experiments.network_authority.run_resolution_preconnected_case \
        "${common_arguments[@]}" \
...
python3 -m experiments.network_authority.record_resolution_indirection \
...
python3 -m experiments.network_authority.verify_resolution_indirection \
  "$output_directory"

python3 -c \
  'import json,sys; result=json.load(open(sys.argv[1], encoding="utf-8")); raise SystemExit(result["conclusion"] != "resolution-indirection-slice-matched")' \
